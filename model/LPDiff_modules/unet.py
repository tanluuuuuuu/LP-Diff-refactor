...
class UNet(nn.Module):
    ...
    def forward(self, x, time):
        # Pad input to be divisible by 2^num_downsamples
        _, _, h, w = x.shape
        stride = 2 ** (len(self.channel_mults) - 1)  # 2^4 = 16
        ...
        x = x[:, :, padding[2]:padding[2]+orig_size[0],
              padding[0]:padding[0]+orig_size[1]]
        return x


if __name__ == '__main__':
    model = UNet(in_channel=6, out_channel=3, inner_channel=64, norm_groups=32, channel_mults=(
        1, 2, 4, 8, 8), attn_res=(16,), res_blocks=2, dropout=0.2, with_noise_level_emb=True, image_size=128)
